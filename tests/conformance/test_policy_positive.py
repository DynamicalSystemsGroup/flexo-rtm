...
"""Acceptance criterion I2: approver with role R, attribute K=V, scoped to S, IS
authorized to emit rtm:SatisfactionAttestation for aspect X iff a Policy P
matches all five.
"""
...
<https://rtm.example/membership/1>
    ...
    rtm:hasAttribute <https://rtm.example/membership/1/attr/clearance> .

<https://rtm.example/membership/1/attr/clearance>
    a rtm:Attribute ;
...
def test_matching_policy_authorizes_attestation() -> None:
    result = check_attestation_authorization(
        attestation_graph=_g(ATTESTATION),
        projection_graph=_g(PROJECTION),
        policy_graph=_g(POLICY),
    )
    assert result.authorized, f"expected authorized, got:\n{result.report_text}"
