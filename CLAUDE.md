...
- **v0.1 `vc_di` diverges from W3C VC-DI 2.0 in three ways** ([#27](https://github.com/DynamicalSystemsGroup/flexo-rtm-research/issues/27)). Live cross-impl test against W3C eddsa-rdfc-2022 reference vectors surfaces: (1) base64 vs multibase-base58btc `proofValue` encoding; (2) flat-triples read of the proof block vs W3C credentials/v2 context's JSON-LD 1.1 `@graph` container; (3) raw-canonical-bytes signing payload vs W3C's `SHA-256(docCanonical)‖SHA-256(proofConfigCanonical)`. Live test [`tests/integration/sign/test_live_vc_di.py`](tests/integration/sign/test_live_vc_di.py) marked `@pytest.mark.xfail(strict=True)` pending resolution. v0.1 ships a non-W3C-conformant `eddsa-rdfc-2022` cryptosuite — cross-impl interop with Digital Bazaar / Spruce / etc. requires the issue's Option A.
- **U3 implementation can't verify non-HEAD git commits** ([#28](https://github.com/DynamicalSystemsGroup/flexo-rtm-research/issues/28)). [`check_git_commit_exists`](oracle/src/oracle/uri/fetch.py) uses `git ls-remote <repo>` and checks whether the commit appears in the output, but `ls-remote` returns ONLY refs (branches, tags, HEAD) — not arbitrary historical SHAs. Surfaced by the live-test sweep against cpython's initial commit. Acceptance criterion U3 ("fetching `rtm:hasGitRepo` + `rtm:hasGitCommit` MUST succeed in audit mode") cannot hold for real-world historical commits with the current implementation. Live test [`test_known_commit_on_public_repo_resolves`](tests/integration/dereferenceable/test_git_commit_resolves.py) marked `@pytest.mark.xfail(strict=True)` pending resolution.
